# This R script is available at https://doi.org/10.17605/osf.io/znw9c.
# The t_test_all function performs t-test and/or its Bayesian alternative, and
# calculates effect sizes and their confidence or credible intervals.
# This function requires tidyverse, effectsize, BayesFactor, and HDInterval packages.
...
t_test_all <- function(
    ...
    conf.level = 0.95,
    pd = F, bf = F, cor = T,
    mean_x_EAP = F, mean_x_MAP = F, mean_x_MED = F,
    diff_EAP = F, diff_MAP = F, diff_MED = F,
    cohens_d = NULL, cohens_d_EAP = F, cohens_d_MAP = F, cohens_d_MED = F,
    cohens_dz = T, cohens_dz_EAP = F, cohens_dz_MAP = F, cohens_dz_MED = F,
    rscale_est = Inf, rscale_bf = "medium",
    iterations = 10000, map_density_n = 512, verbose = T,
    ...
){
  ...
  # Function
  map_dens <- function(z) density(z, n = map_density_n)$x[which.max(density(z, n = map_density_n)$y)]
  ...
  if(
    ...
    if(is.null(y)){
      ...
      if(mean_x_MAP) out$mean_x_MAP <- map_dens(mcmcsample[,"mu"])
      if(mean_x_MED) out$mean_x_MED <- median(mcmcsample[,"mu"])
      if(cohens_d_EAP) out$cohens_d_EAP <- mean(mcmcsample[,"delta"])
      if(cohens_d_MAP) out$cohens_d_MAP <- map_dens(mcmcsample[,"delta"])
      ...
      if(ci == "bayes_hdi"){
        tmp <- HDInterval::hdi(mcmcsample[,"mu"], credMass = conf.level)
        out$mean_x_lower <- tmp[1]
        out$mean_x_upper <- tmp[2]
        if(cohens_d | cohens_d_EAP | cohens_d_MAP | cohens_d_MED){
          tmp <- HDInterval::hdi(mcmcsample[,"delta"], credMass = conf.level)
      ...
      if(diff_MAP) out$diff_MAP <- map_dens(mcmcsample_diff)
      if(diff_MED) out$diff_MED <- median(mcmcsample_diff)
      if(cohens_dz_EAP) out$cohens_dz_EAP <- mean(mcmcsample[,"delta"])
      if(cohens_dz_MAP) out$cohens_dz_MAP <- map_dens(mcmcsample[,"delta"])
      ...
      if(cohens_d_EAP|cohens_d_MAP|cohens_d_MED|
        ...
        if(cohens_d_MAP) out$cohens_d_MAP <- map_dens(mcmcsample_between[,"delta"])
      ...
      if(ci == "bayes_hdi"){
        ...
        if(cohens_d | cohens_d_EAP | cohens_d_MAP | cohens_d_MED){
          tmp <- HDInterval::hdi(mcmcsample_between[,"delta"], credMass = conf.level)
          out$cohens_d_lower <- tmp[1]
          out$cohens_d_upper <- tmp[2]
        }
        if(cohens_dz | cohens_dz_EAP | cohens_dz_MAP | cohens_dz_MED){
      ...
      if(diff_MAP) out$diff_MAP <- map_dens(mcmcsample[,"beta (x - y)"])
      ...
      if(ci == "bayes_hdi"){
        tmp <- HDInterval::hdi(mcmcsample[,"beta (x - y)"])
        out$diff_lower <- tmp[1]
        out$diff_upper <- tmp[2]
        if(cohens_d | cohens_d_EAP | cohens_d_MAP | cohens_d_MED){
          tmp <- HDInterval::hdi(mcmcsample[,"delta"], credMass = conf.level)
    ...
    if(pd){
      ...
      out$pd <- pd_posterior(mcmcsample[,"delta"])
