#' Internal Helper Functions for anovakun
#'
#' This file contains internal functions used within the anovakun package.
#' These functions are not exported and should not be used directly by end users.
#'
#' @keywords internal
"_PACKAGE"
...
uni.long <- function(dataset, design, ..., long = FALSE){
  ...
  if(long){
    ...
    misv <- suppressWarnings(as.numeric(sapply(dataset[, ncol(dataset)], function(x) as.vector(x))))
    dat <- cbind(data.frame(lapply(dataset[, 1:(maxfact+1), drop = FALSE], function(x) factor(x, levels = unique(x)))), misv)
    ...
    attributes(flev) <- NULL
    if(!anyNA(misv)){
      miscase <- 0
    }else{
      misid <- dataset[is.na(misv), 1]
      dat <- dat[rowSums(sapply(misid, function(x) x == dat[, 1])) == 0, ]
    ...
    misv <- suppressWarnings(as.numeric(sapply(dataset[, (betlen+1):ncol(dataset)], function(x) as.vector(x))))
    cdata <- array(misv, c(nrow(dataset), ncol(dataset) - betlen))
    compcase <- complete.cases(cdata)
    dataset <- dataset[compcase, ]
    ...
    if(betlen > 0){
      betlab <- data.frame(lapply(dataset[, 1:betlen, drop = FALSE], function(x) factor(x, levels = unique(x))))
      for(i in 1:betlen){
...
ci.calc <- function(dat, design, factnames = NA, conf.level = 0.95, cilmd = FALSE, cilm = FALSE, cind = FALSE, cin = FALSE, ciml = FALSE, cipaird = FALSE, cipair = FALSE){
  ...
  if(cind || cin){
    if(withlen == 0){
      ...
      wdat <- lapply(caldat, function(x) data.frame(matrix(x$y[order(eval(parse(text = paste0("x[, ", c(1, (maxfact+1):(betlen+2)), "]"))))], length(x$y)/comlev, comlev)))
  ...
  if(ciml){
    loadstate <- search()
    pkchk <- requireNamespace("lmerTest", quietly = TRUE)
    if(pkchk == FALSE) stop("Please install lmerTest package to use ciml option!!")
    def.contr <- suppressWarnings(lmerControl()$checkControl)
...
epsilon.calc <- function(dat, design, mau = FALSE, har = FALSE, iga = FALSE, ciga = FALSE, lb = FALSE, gg = FALSE, hf = FALSE,
  ...
  combmat <- expand.gmatrix(lapply(repmat - 1, function(x) 0:x))[, rev(1:withlen), drop = FALSE]
  ...
  ortho.coef <- t(ortho.helm[, 2:rl, drop = FALSE])
  ...
  HF.ep <- ((cellN - ol + 1) * gmd * GG.ep - 2) / (gmd * (cellN - ol - gmd * GG.ep))
  ...
  if(esboot){
    ...
    if(mau){
      ...
      menL3 <- log(ifelse(menL3 < 0, NA, menL3)) * ((cellN-ol) * gmd / 2)
...
anova.modeler <- function(dat, design, factnames = NA, type2 = FALSE, dmat = NULL, flev = NULL, cellN = NULL, full.elem = NA,
  ...
  if(!is.null(bet.mse)){
    ss.col[mse.row] <- bet.mse[[2]]
    df.col[mse.row] <- bet.mse[[3]]
    ss.qT <- gss.qT
  }else{
    ss.qT <- sum(ss.col[-eflen])
  }
  ms.col <- ss.col / df.col
  f.col <- ms.col[1:length(ms.col)] / ms.col[f.denomi]
  ...
  f.col <- f.col / mfv
  p.col <- pf(f.col, df.col, df.col[f.denomi], lower.tail = FALSE)
  esmat <- NULL
  esdenomis <- NULL
  es.df.adjs <- NULL
  es.mse.adjs <- NULL
  ...
  if(esboot){
    ...
    if(is.na(inter)){
      if(withlen > 0) epsitab$Effect <- epsi.effect[(2 * sum(withlen == 1)):length(epsi.effect)]
      ...
      if(charmatch(inter, strsplit(design, "")[[1]]) < charmatch("s", strsplit(design, "")[[1]])){
        if(iga || ciga) interepsi <- rep(NA, 11)
...
mod.Bon <- function(dat, design, taref, bet.mse = NA, factlabel = NA, factnames = NA, type2 = FALSE, holm = FALSE, hc = FALSE,
  ...
  factlevels <- factlevels[!(factlevels == factlevels[1] | factlevels == factlevels[length(factlevels)])]
  ...
  if(welch){
    ...
    bontab <- bontab[order(bontab$p.value), ]
  ...
  if(holm){
    ...
    if(s2d){
      ...
      p.criteria <- c(shaf.value[2], shaf.value[2:length(shaf.value)])
  ...
  if(criteria){
    if(hc){
      bontab <- transform(bontab, "criteria" = 1 - (1 - alpha) ^ (1/p.criteria))
      ...
      bontab <- transform(bontab, "criteria" = alpha/p.criteria)
    }
    bon.sign <- ifelse(cummin(bontab$p.value < bontab$criteria), paste(bon.differ, "*", sep = " "), paste(bon.equal, " ", sep = " "))
  }else{
    if(hc){
      bontab <- transform(bontab, "adj.p" = pmin(1, cummax((1-(1-bontab$p.value)^p.criteria))))
      if(holm == TRUE) bon.info1 <- paste0("== Holm's Sequentially Rejective Sidak Procedure ==")
      else bon.info1 <- paste0("== Holland-Copenhaver's Improved Sequentially Rejective Sidak Procedure", shaf.meth)

      if(length(bet.mse) == 1) bon.info1 <- append(bon.info1, "*** CAUTION! This procedure might be inappropriate for dependent means. ***")
    }else{
      bontab <- transform(bontab, "adj.p" = pmin(1, cummax(bontab$p.value * p.criteria)))
    }
  ...
  if(sig.num > 3){
    ...
    if(withlen == 0){
      ...
      if(!all(is.na(simepsi))){
        simepsi$Effect <- subsource[simefpos]
        simepsi <- simepsi[!is.na(simepsi$df), ]
      }
...
boot.esci <- function(dat, design, factnames = NA, type2 = FALSE, nopost = FALSE, mainresults = NULL, postresults = NULL,
  ...
  if(auto){
    autov <- list((mainresults$epsitab$p >= 0.10)[min(withlen, 2):nrow(mainresults$epsitab)])
  }else{
    autov <- NULL
  }
  ...
  if(length(int.sig.source) > 0){
    ...
    postype <- lapply(int.sig.source, function(x) sum(is.na(postresults[[x]]$simepsi)) == 1)
...
table.out <- function(dattable, titleinfo = NULL, subinfo1 = NULL, subinfo2 = NULL, use.header = NULL, smargin = nrow(dattable),
  ...
  dcnum <- ncol(dattable)
  drnum <- nrow(dattable)
  numlab <- sapply(1:dcnum,function(x) is.numeric(dattable[, x]))
  bmat <- do.call("cbind", apply(dattable, 2, function(x) nchar(as.character(x), type = "bytes"), simplify = FALSE))
  bmat <- replace(bmat, is.na(bmat), 2)
  wmat <- do.call("cbind", apply(dattable, 2, function(x) nchar(as.character(x), type = "width"), simplify = FALSE))

  mblab <- colSums(bmat == wmat) != drnum
  tabline <- pmax(3, sapply(1:dcnum, function(x) ifelse(numlab[x], max(nchar(sprintf("%.4f", dattable[, x]), type = "bytes")), max(bmat[, x]))) + tnick)
  if(is.null(use.header)){
    tabhead <- nchar(names(dattable), type = "bytes") + 1
    tabheadw <- nchar(names(dattable), type = "width") + 1
    hmmod <- tabhead != tabheadw
  }else{
    tabhead <- nchar(use.header, type = "bytes") + 1
    tabheadw <- nchar(use.header, type = "width") + 1
    hmmod <- tabhead != tabheadw
  }
  ...
  mainline <- sum(tabline) + length(tabline) - sum(apply(bmat - wmat, 2, function(x) max(x, na.rm = TRUE)))

  headmargin <- sapply(tabline, function(x) paste0("%", x, "s"))
  bodymargin <- headmargin
  coltype <- sapply(1:dcnum, function(x) is.factor(dattable[, x]) | is.character(dattable[, x]))
  coltype[strpos] <- TRUE
  bodymargin[!coltype] <- paste0("%", tabline[!coltype], ".4f")
  bodymargin[leftal] <- gsub("%", "%-", bodymargin[leftal])
  bmmat <- array(rep(bodymargin, each = drnum), c(drnum, dcnum))

  if(any(hmmod, na.rm = TRUE)) headmargin[hmmod] <- paste0("%", tabline[hmmod] + tabhead[hmmod] - tabheadw[hmmod], "s")
  if(any(mblab, na.rm = TRUE)){
    for(i in grep(TRUE, mblab)){
      bmmat[, i] <- paste0("%", tabline[i] - (max(bmat[, i] - wmat[, i]) - (bmat[, i] - wmat[, i])), "s")
      headmargin[i] <- paste0("%", tabline[i] - (max(bmat[, i] - wmat[, i]) - (tabhead[i] - tabheadw[i])), "s")
    }
  }
  if(!is.null(titleinfo)){
  ...
  for (i in 1:drnum){
    for(j in 1:dcnum){
      if(is.element(j, na.erase)){
        cat(replace(sprintf(bmmat[i, j], dattable[i, j]), is.na(dattable[i, j]), sprintf(headmargin[j], "")))
      }else{
        cat(sprintf(bmmat[i, j], dattable[i, j]))
      }
      cat(sep, sep = "")
    }
    cat("\n")
